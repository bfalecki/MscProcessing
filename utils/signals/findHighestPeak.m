function [selected_peaks,selected_locs] = findHighestPeak(matrix)
%UNTITLED this function finds 1 highest maksima
...
for k = 1:size(matrix, 2)
    column = matrix(:,k);
    [~, locs] = findpeaks(column,"NPeaks",1,"SortStr","descend");
    if(isscalar(locs))
